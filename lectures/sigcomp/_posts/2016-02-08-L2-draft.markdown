...
This can be seen by the fact that the always-left recursive path is rooted at Fib(n) and most recurse one step at a time from Fib(n) to Fib(n-1) to ... to Fib(1).
...
If the tree is complete, then it is indeed the cast that you have $$2^n$$ nodes and thus the correct time complexity, but in our case there is no reason to believe that it is.
...
From there, you can use the initial conditions $$T_0 = T_1 = 1$$ to determine the constant values and then you have your Big-O time complexity, which is approximately $$\mathcal{O}(1.6^n)$$. That's a lot of extra work just to confirm that the time complexity is exponential, but you learned something. As an interesting aside, this exact approach works for linear ordinary differential equations, they just use a different generating function, $$e^{nx}$$.
...
As it turns out, you end up recomputing a lot of Fibonacci numbers in the recurssive approach.
...
If you imagine taking the always-left recursive path down the binary tree discussed before, but then memorize results on the way up, then you never end up taking any other path except to realize you memorized it.
...
As a next example let's us a problem from [Codeforces](http://codeforces.com).
...
It turns out you're computing all the permutations of length $$1, \cdots, n$$ with repeated entires.
That mean we're dealing with a permutation with repetitions.
If you try to run the above code on anything about say $$30$$ you'll notice that it takes much longer than half a second.
If you use our counting principles we can quickly see that the time complexity for this algorithm is $$\mathcal{O}(2^n)$$, exponential.
...
## Heuristics Using Problem Bounds
Big-O notation offers us a powerful way to determine time complexity at scale, and with the constant multiplier we can do decently well at lower values as well.
However, in a competitive setting, there additional information which is crucial in determining if your solution will work.
Almost every problem will incorporate some kind of variable bound.
Whether it's on the size of an array, the number of vertices, or what have you.
...
Of course, this is a broad generalization of the reality, and if your constant multiplier is reducing an eight million operation count worst case down to two million, then things just might work out.
...
A naive solution is to generate all the permutations, store them in memory, and then print them out for each query.
...
Try the below naive solution out for yourself, the input it assumed to be two numbers $$n$$ then $$q$$, a lexicographically ordered string, and then $$q$$ numbers that are smaller than $$n$$.
...
Hopefully after this you can agree with me that Big-O notation is valuable tool in understanding the cost of a particular algorithm.
We didn't discuss space complexity in any great depth, but all of the same principles apply, you just measure space instead of time.
...
Ultimately that is the point, being able to make quick and powerful judgments about potential solutions so that you can make better use of your development time, as well as your machine time.
